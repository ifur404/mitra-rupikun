...
import { productTable, tagTable, userTable } from "~/drizzle/schema";
...
export async function action(req:ActionFunctionArgs) {
  ...
  try {
    if(intent==="ADD_DATA"){
      ...
      await mydb.insert(productTable).values({
        ...data,
...
function AddData(){
  return <SheetAction title="Add Data" triger={<Button><Plus className="mr-2"/> Add Data</Button>}>
    ...
    <div>
      ...
      <InputCurrency name="price" id="price" defaultValue="0"  />
    </div>
    <div>
      <Label htmlFor="price_sell">Price Sell</Label>
      <InputCurrency name="price_sell" id="price_sell" defaultValue="0" />
    </div>
